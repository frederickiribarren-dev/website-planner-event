...
<form method="POST" action="{{ route('login') }}" class="space-y-6">
    ...
    <div class="space-y-2">
        ...
        <x-text-input id="email" class="w-full border-slate-200 rounded-[1.25rem] px-6 py-4 bg-white text-slate-900 focus:ring-4 focus:ring-teal-500/10 focus:border-[#335C5C] transition-all duration-300 placeholder:text-slate-300 text-sm shadow-sm hover:border-slate-300" 
    ...
    <div class="space-y-2">
        <div class="flex justify-between items-end px-1">
            <x-input-label for="password" :value="__('Contraseña')" class="flex items-center gap-2.5 text-xs font-black text-[#335C5C] uppercase tracking-widest" />
            @if (Route::has('password.request'))
                <a class="text-[10px] text-slate-400 hover:text-[#335C5C] font-bold transition-colors uppercase tracking-wider" href="{{ route('password.request') }}">
    ...
    <div class="flex items-center px-1">
        <label for="remember_me" class="inline-flex items-center cursor-pointer group">
            <input id="remember_me" type="checkbox" class="rounded-md border-slate-300 text-[#335C5C] shadow-sm focus:ring-[#335C5C]/20 transition-all" name="remember">
            <span class="ms-2 text-sm text-slate-500 group-hover:text-slate-700 transition-colors">Mantener sesión iniciada</span>
        </label>
    </div>
    ...
    <p class="text-center text-sm text-slate-500 mt-10 pt-6 border-t border-slate-100">
        ...
        <a href="{{ route('register') }}" class="text-teal-600 font-bold hover:text-teal-700 transition-colors">
            Regístrate aquí
        </a>
    </p>
</form>
